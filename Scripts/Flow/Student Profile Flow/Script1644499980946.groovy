...
String testCaseName = 'Student Profile Report'
def filterBy = ['profile', 'student']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\375_students.xlsx', 'profile', filterBy)
...
for (def row : data)
	{
	    ...
	    WebUI.callTestCase(findTestCase('Generic/Student Profile General'), [('testCaseName') : testCaseName, ('testCaseData') : row], 
	        FailureHandling.CONTINUE_ON_FAILURE)
	}
