...
String testCaseName = 'Student Paynow Report'
def filterBy = ['Paynow', 'student']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\Student Paynow.xlsx', 'Paynow', filterBy)
...
for (def row : data)
	{
		...
		WebUI.callTestCase(findTestCase('Test Cases/Generic/Student Paynow general'), [('testCaseName') : testCaseName, ('testCaseData') : row],
		FailureHandling.CONTINUE_ON_FAILURE)
	}
