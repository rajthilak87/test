...
String testCaseName = 'Add Milestone Report'
def filterBy = ['signin', 'tutor']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\Milestone.xlsx', 'Milestone', filterBy)
...
for (def row : data)
	{
		...
		WebUI.callTestCase(findTestCase('Test Cases/Generic/Tutor Add Milestone General'), [('testCaseName') : testCaseName, ('testCaseData') : row],FailureHandling.CONTINUE_ON_FAILURE)
	}
		String currentUrl = WebUI.getUrl(FailureHandling.CONTINUE_ON_FAILURE).toString()
		if (currentUrl.contains('class-details'))
			{	
				filterBy = ['Milestone', 'tutor']
				data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\Milestone.xlsx', 'Milestone_list', filterBy)
				WebUtil.stopOnEmptyData(data)
				for (def row : data)
					{

				WebUI.callTestCase(findTestCase('Generic/Tutor Add Milestone General one'), [('testCaseName') : testCaseName, ('testCaseData') : row],FailureHandling.CONTINUE_ON_FAILURE)
			}
			}
			else
			{
