...
String testCaseName = 'Tutor Reassign Report'
def filterBy = ['reassign', 'tutor']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\tutor_reassign.xlsx', 'reassign', filterBy)
...
for (def row : data)
	{			
		...
		if (currentUrl.contains('dashboard'))
			{
				//WebUI.click(findTestObject('Object Repository/bookaclass/enrol/camp'))
				//WebUI.click(findTestObject('Object Repository/tutor-add-class/add_class_link'))
				WebUI.callTestCase(findTestCase('Generic/Tutor Reassign General'), [('testCaseName') : testCaseName, ('testCaseData') : row],FailureHandling.CONTINUE_ON_FAILURE)		
			}	
