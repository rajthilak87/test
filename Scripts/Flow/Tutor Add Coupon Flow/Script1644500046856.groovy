...
String testCaseName = 'Tutor Addcoupon Report'
def filterBy = ['add_coupon', 'tutor']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\tutor_add_coupon.xlsx', 'add_coupon', filterBy)
...
for (def row : data)
	{
		...
		if (currentUrl.contains('dashboard'))
			{
				WebUI.callTestCase(findTestCase('Generic/Tutor Add Coupon General'), [('testCaseName') : testCaseName, ('testCaseData') : row], 
					FailureHandling.CONTINUE_ON_FAILURE)
			} 
