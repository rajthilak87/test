...
String testCaseName = 'Add class Report'
def filterBy = ['add_class', 'tutor']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\tutor_add_class.xlsx', 'add_class', filterBy)
...
for (def row : data)
	{
		...
		if (currentUrl.contains('dashboard'))
			{
				WebUI.callTestCase(findTestCase('Test Cases/Generic/Tutor_add_class_check_category'), [('testCaseName') : testCaseName, ('testCaseData') : row], 
					FailureHandling.CONTINUE_ON_FAILURE)
			}
			else
				{
