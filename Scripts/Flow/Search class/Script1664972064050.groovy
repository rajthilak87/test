...
def filterBy = ['search_class', 'student']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\search_class.xlsx', 'search', filterBy)
...
for (def row : data)
	{
		...
		//WebUI.delay(5)
		WebUI.callTestCase(findTestCase('Generic/Search class General'), [('testCaseName') : testCaseName, ('testCaseData') : row], 
        FailureHandling.CONTINUE_ON_FAILURE)
	}
