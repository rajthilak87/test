...
String testCaseName = 'Student Booking Check'
def filterBy = ['signin', 'student']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\student-enrolling-check.xlsx', 'enroll', filterBy)
...
for (def row : data)
	{
		...
		WebUI.callTestCase(findTestCase('Test Cases/Generic/Student Booking Check General'), [('testCaseName') : testCaseName, ('testCaseData') : row],
		FailureHandling.CONTINUE_ON_FAILURE)
	}
