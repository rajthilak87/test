...
String testCaseName = 'Camp Book Report'
def filterBy = ['class_booking', 'student']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\Student Book Camp.xlsx', 'camp_booking', filterBy)
...
for (def row : data)
	{
		...
		WebUI.delay(5)
		WebUI.callTestCase(findTestCase('Generic/Student Book Camp General'), [('testCaseName') : testCaseName, ('testCaseData') : row], 
        FailureHandling.CONTINUE_ON_FAILURE)
	}
