...
String testCaseName = 'Enroll Class Report'
def filterBy = ['profile', 'tutor']
def data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\tutor-enrolling.xlsx', 'enroll', filterBy)
...
for (def row : data)
	{
		...
		if (currentUrl.contains('dashboard'))
			{	
				...
				data = WebUtil.getData(GlobalVariable.projectRootPath + '\\TestDataXls\\tutor-enrolling.xlsx', 'enroll_list', filterBy)
				WebUtil.stopOnEmptyData(data)
				for (def row : data)
					{
				WebUI.click(findTestObject('Object Repository/tutor-add-class/add_class_link'))
				WebUI.callTestCase(findTestCase('Generic/Tutor Enroling General'), [('testCaseName') : testCaseName, ('testCaseData') : row],FailureHandling.CONTINUE_ON_FAILURE)
			}
			}
